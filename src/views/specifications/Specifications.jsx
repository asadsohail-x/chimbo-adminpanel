...
import {
  ...
  delAsync,
  filter,
} from "../../redux/specifications/specifications.slice";
...
const Modal = ({ open, handleClose, submit }) => {
  return (
    ...
    >
      <DialogTitle id="alert-dialog-title">Delete Specification?</DialogTitle>
      <DialogContent>
        <DialogContentText id="alert-dialog-description">
          If you choose to delete the specification, all the Users with the
          specified specification will be removed. Are you sure you want to
          proceed?
        </DialogContentText>
      </DialogContent>
      <DialogActions>
...
const Specifications = () => {
  ...
  const specifications = useSelector(
    (state) => state.specifications.specifications
  );
  const isLoading = useSelector((state) => state.specifications.isLoading);
  ...
  const editItem = (id) => {
    if (createDraft(id)) {
      const { name, type } = specifications.find(({ _id }) => id === _id);
      setNewItemName(name);
      setNewItemType(type);
      setIsNewItem(false);
    } else {
      ...
      <Container style={{ maxWidth: 1000 }}>
        ...
        >
          ...
          {specifications.length > 0
            ...
            : !isNewItem && <NoItems name="Specifications" />}
        </Box>
      </Container>
    </Box>
  );
