...
import {
  ...
  delAsync,
  filter,
} from "../../redux/listingTypes/listingTypes.slice";
...
const Modal = ({ open, handleClose, submit }) => {
  return (
    ...
    >
      <DialogTitle id="alert-dialog-title">Delete Listing Type?</DialogTitle>
      <DialogContent>
        <DialogContentText id="alert-dialog-description">
          If you choose to delete the Listing Type, all the Listings with the
          specified listing type will be removed. Are you sure you want to
          proceed?
        </DialogContentText>
      </DialogContent>
      <DialogActions>
...
const ListingTypes = () => {
  // Redux States
  const listingTypes = useSelector((state) => state.listingTypes.listingTypes);
  const isLoading = useSelector((state) => state.listingTypes.isLoading);
  ...
  const editItem = (id) => {
    if (createDraft(id)) {
      setNewItemName(listingTypes.find(({ _id }) => id === _id).name);
      setIsNewItem(false);
    } else {
      // TODO: Add an alert message here
      console.log(
      ...
      <Container style={{ maxWidth: 1000 }}>
        ...
        >
          ...
          {listingTypes.length > 0
            ...
            : !isNewItem && <NoItems name="Listing Types" />}
        </Box>
      </Container>
    </Box>
  );
